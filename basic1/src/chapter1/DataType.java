...
//		int1 = 2_147_483_648;
		...
		char char1 = 97; // a를 10진수로 표현한 방법
		System.out.println(char1);
		
		// 논리형 데이터 타입 : 참과 거짓 갑슬 가지는 데이터 타입
		...
		// 형 변환 : 데이터 타입이 서로 다른 변수를 옮겨 닮는 것
		
		// 강제 형 변환(명시적 변환) : 큰 데이터 타입의 변수를 작은 데이터 타입의 변수에 옮겨 닮을 때 발생
		
		// 자동 형 변환(묵시적 변환) : 작은 데이터 타입의 변수를 큰 데이터 타입의 변수에 옮겨 닮을 때 발생
		...
		char char3 = 100;
		short short2 = (short)char3; // 같은 byte지만 문자형이라서 묵시적으로 형변환이 안되기때문에 명시를 해줘야 한다.
		int int2 = char3;
		...
		int[] numbers2 = {1, 2, 3, 4, 5}; // 배열 초기화 방법
...
//		System.out.println(numbers3[99]);
		...
		System.out.println(statement.equals("  MY name is loki  "));// 위에서 선언한 결과값이 맞는지 아닌지 구분해주는 코드
		...
		statement = "내가 그린 기린 그림";
		System.out.println(statement.indexOf("기린"));
		System.out.println(statement.indexOf("호랑이"));
		...
		System.out.println(statement.substring(3,9));// "" 번 부터 "" 번 까지 가져오는 코드
		System.out.println(statement.substring(3));// 3번째 요소부터 추출해서 마지막번호까지 가져온다
