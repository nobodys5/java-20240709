...
SELECT * FROM nurse WHERE id = '1' AND password = '1234';

-- 회원가입 
-- 최종사용자 (이름, 아이디, 비밀번호, 비밀번호확인, 전화번호, 인증번호)
-- 요양사 테이블에 레코드(아이디, 암호화된 비밀번호, 전화번호, 가입경로, 이름)를 생성
-- 필요한 추가 데이터 : 가입경로 (화면에서 가져옴), 암호화된 비밀번호 (서버에서 생성)

-- 레코드 삽입시 아이디 중복 검증
SELECT * FROM nurse WHERE id = 'samu';
-- 레코드 삽입시 전화번호 중복 검증
SELECT * FROM nurse WHERE tel_number = '01012345678';
-- 절차상에 인증된 전화번호만 삽입 가능
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND auth_number = '0125';
...
UPDATE tel_auth SET auth_number = '0125' WHERE tel_number = '01012345678';

INSERT INTO nurse VALUES ('samu', 'qwerasdzxc', '홍길동', '01012345678', 'HOME');
...
-- 고객 테이블에서 이름이 포함되어 있는 레코드들을 페이지 번호에 해당하는 위치부터 10개의 고객 수를 반환
-- 필요한 추가 데이터 : 페이지 번호에 해당하는 시작위치 (서버가 생성)

SELECT C.customer_number, C.name, C.birth, C.area, N.name
FROM customer C INNER JOIN nurse N ON C.charger = N.id
...
# 1 번과 charger 를사용하는 사람이 같은지 확인하는것
SELECT * FROM customer WHERE customer_number = 3 AND charger = 'samu';
DELETE FROM customer WHERE customer_number = 4;
...
INSERT INTO customer(name, area, charger, profile_image, birth, address) VALUES ('이영희', '부산광역시', 'samu', null, '590826', '부산광역시 부산진구');
INSERT INTO customer(name, area, charger, profile_image, birth, address) VALUES ('신슬기', '부산광역시', 'samu', null, '591826', '부산광역시 부산진구');
INSERT INTO customer(name, area, charger, profile_image, birth, address) VALUES ('김덱스', '부산광역시', 'samu', null, '592826', '부산광역시 부산진구');
INSERT INTO customer(name, area, charger, profile_image, birth, address) VALUES ('김울산', '부산광역시', 'samu', null, '593826', '부산광역시 부산진구');

-- 담당자(요양사) 검색
-- 최종 사용자 (이름)
-- 최종 사용자 (이름) => 아이디, 이름, 전화번호
-- 요양사 테이블에서 이름을 기준으로 입력한 이름과 같은 레코드르 조회
-- 필요 추가 데이터 : X
...
-- 필요 추가 데이터 : 고객 번호 (화면에서)
SELECT C.customer_number, C.profile_image, C.name, C.birth, N.name, N.id, C.address
FROM customer C INNER JOIN nurse N ON C.charger = N.id
WHERE C.customer_number = 3; 
...
FROM customer_management CM LEFT JOIN goods G ON CM.used_goods = G.goods_number
...
INSERT INTO customer_management VALUES (1, '2024-07-26', '최초 방문', null, 1 );
...
-- 고객 수정
-- 최종 사용자 (고객 사진, 고객 이름, 생년월일, 담당자, 주소)
-- 고객 테이블에서 특정 고객에 대한 레코드(고객 사진, 고객 이름, 생년월일
-- 필요 추가 데이터 : 고객 번호(화면에서)
...
INSERT INTO nurse VALUES ('sa', 'qwerasdzxc', '홍길동', '01012345678', 'HOME');
SELECT * FROM nurse;
...
INSERT INTO tel_auth VALUES (tel_number, auth_number);

-- 인증번호 확인 
-- 최종사용자 (인증번호)
-- 전화번호 인증 테이블에서 전화번호에 해당하는 인증번호가 사용자가 입력한 인증번호와 같은지 비교
-- 필요한 추가 데이터 : 전화번호 (화면에서 가져옴)
SELECT * FROM tel_auth WHERE tel_number = '1' AND auth_number = '1';
