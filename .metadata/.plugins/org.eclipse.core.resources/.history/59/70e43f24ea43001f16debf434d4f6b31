...
//		ExtendsClass<MyClass> instance3 = new ExtendsClass<MyClass>();
		...
		wildMethod(list2);
		List<SuperClass> list3 = new ArrayList<SuperClass>();
		wildMethod(list3);
		
		List<MyClass> list4 = new ArrayList<MyClass>();
		List<SubClass> list5 = new ArrayList<SubClass>();
		
		// ? extends MyClass 로 제한이 걸렸기 때문에
		// MyClass 혹은 MyClass를 상속한 클래스만 매개변수로 전달이 가능
		extendsMethod(list4);
//		extendsMethod(list3); super클래스는 호출할수 없음
//		extendsMethod(list2); 아예 다른 타입은 제한이 걸려서 호출할수 없음
		
		// ? super MyClass 로 제한이 걸렸기 때문에
		// MyClasss 혹은 MyClass의 부모 클래스만 매개변수로 전달할 수 있음
		superMethod(list4);
//		superMethod(list3);
	}
